...
import queue
import numpy as np
from lib.get_window.get_window import GetWindow
import cv2
...
fucking_window = GetWindow("EVE Launcher")
capture = dxcam.create(region=fucking_window.get_region())
window = pyglet.window.Window(width=960,
                                height=540,
...
def on_draw():
    ...
    if img is None:        
        ...
        img = pyglet.image.ImageData(w, h, 'RGB', _buf, pitch=-w*3)
    else:
        _buf_np[:] = frame.reshape(-1)
        img.set_data('RGB', -w * 3, _buf) 
...
if __name__ == "__main__":
    try:
        capture.start(target_fps=INFER_FPS)
